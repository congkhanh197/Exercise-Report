...
$$ p \vee q \equiv \neg p \rightarrow q $$
$$1 \hspace{2 cm} \neg p \hspace{2 cm} premise$$
$$2 \hspace{2 cm} \neg p \rightarrow q \hspace{1.3cm} premise$$
$$3 \hspace{2 cm} q \hspace{2.8 cm} \rightarrow e$$
$$\begin{array}{|c|}
\hline 
{4 \hspace{2.5cm} $\neg$q \hspace{3cm} Assumption}\\
{5 \hspace{2.5cm} $\bot$ \hspace{3cm} $\wedge$i 3,4}\\
\hline
\end{array}$$
$$6 \hspace{2cm} \neg \neg q \hspace{2cm} \wedge i \hspace{0.1cm} 5,6$$



\textbf{\large\hspace{0.5cm} Exercise 1.5:}
1.5 7 c)\\
$F_{\o_{3}}$ = {{(T, T, T),(T, F, T),(F, T, T),(F, T, F),(F, F, T)}\\
	Chúng ta sử dụng F của $\O_{3}$ để tìm CNF\\
	\begin{center}
		(T, T, T) $\Rightarrow$ ($\neg$r $\vee$ $\neg$s $\vee$ $\neg$q)\\	
		(T, F, T) $\Rightarrow$ ($\neg$r $\vee$ s $\vee$ $\neg$q)\\
		(F, T, T) $\Rightarrow$ (r $\vee \neg$s $\vee$ $\neg$q)\\
		(F, T, F) $\Rightarrow$ (r $\vee$ $\neg$s $\vee$ q)\\
	\end{center}
	Suy ra được CNF của $\O_{3}$ là:\\
	($\neg$r $\vee$ $\neg$s $\vee$ $\neg$q) $\wedge$ ($\neg$r $\vee$ s $\vee$ $\neg$q) $\wedge$ (r $\vee$ $\neg$s $\vee$ $\neg$q) $\wedge$  (r $\vee$ $\neg$s $\vee$ q)\\
\end{document}